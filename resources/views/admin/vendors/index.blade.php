...
@section('content')
      ...
      <div class="px-4">
        ...
        <div class="table-wrapper">
          ...
          <table id="dtBasicExample" class="table table-hover mb-0">
            ...
            <tr deletedid="{{$vendor->id}}">
              <td>{{$vendor->name}}</td>
                <td>{{$vendor->mobile}}</td>
                <td statusactive="{{$vendor->id}}" style="color: @if($vendor->status=='0') red @elseif($vendor->status=='1') green @elseif($vendor->status=='2')#ffc155 @endif">@if($vendor->status=='0')Blocked @elseif($vendor->status=='1') Active  @elseif($vendor->status=='2') Not Active @endif</td>
              ...
              <td> <div>
                ...
                <td>
                    <div>
                      <a href="{{route('admin.vendors.changeStatus')}}" activateid="{{$vendor->id}}" class="vendoractivate btn btn-@if($vendor->status=='0')primary @elseif($vendor->status=='1'||$vendor->status=='2')danger @endif  btn-sm px-2" style="color:white;min-width: 55%;">
                          @if($vendor->status=='0')Activate
                          @elseif($vendor->status=='1'||$vendor->status=='2')Block @endif
                      </a>
                </div>
                </td>
            </tr>
  ...
  <script>
      window.onload = function(){
          ...
          $(document).on('click','.vendoractivate',function (e) {
              var button=this;
              e.preventDefault();
              $.ajax({
                  type:'get',
                  ...
                  success:function (data) {
                      if(data.show==true) {
                          ...
                          $('#message').text(data.message);
                          if(data.action) {
                              $(button).attr('class', 'btn-sm px-2 vendoractivate btn btn-' + data.btn);
